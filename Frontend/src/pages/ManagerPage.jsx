...
export const ManagerPage = () => {
  ...
  const handleResolveDispute = (taskId, resolution) => {
    ...
    if (worker && payAmount > 0) {
      ...
      const newCompletedCount = resolution === 'paid' ? worker.completedCount + 1 : worker.completedCount;
      ...
      addNotification({
        ...
        message: `Task Manager resolved dispute. You received ₦${payAmount.toLocaleString()} for: ${task.title}`,
        taskId: taskId,
    ...
    <div className="space-y-6">
              ...
              return (
                <div key={task.id} className="border border-red-200 rounded-lg p-4 bg-red-50">
                  <div className="mb-3">
                    ...
                    <p className="text-sm text-red-700 mt-1">{task.disputeReason}</p>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    ...
                    <button
                      onClick={() => alert('Escalating to higher management...')}
      ...
      <div className="bg-white rounded-lg p-4">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="font-semibold text-gray-900">Pending Confirmations (6h+)</h2>
                ...
                <div key={task.id} className="border border-orange-200 rounded-lg p-4 bg-orange-50">
                  <div className="mb-3">
                    ...
                    <p className="text-sm text-orange-700">
                      Completed {task.completedAt && formatTimeAgo(task.completedAt)} - No response from poster
                    </p>
                    <p className="text-xs text-gray-500 mt-1">
                      Pay: ₦{task.pay.toLocaleString()} • {task.location}
                    </p>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    {poster && (
                      <div className="text-sm">
                        <p className="font-medium text-gray-700">Poster (No Response)</p>
                        <p className="text-gray-600">{poster.name}</p>
                        <button
                          onClick={() => handleCallUser(poster.id)}
                          className="flex items-center space-x-1 text-blue-600 hover:text-blue-800 mt-1"
                ...
                <div key={task.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Resolved • {worker?.name} • {task.confirmedAt && new Date(task.confirmedAt).toLocaleDateString()}
                    </p>
                  </div>
